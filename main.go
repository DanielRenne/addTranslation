...
import (
	...
	yandex_translate "github.com/DanielRenne/go-yandex-translate"
)
...
func main() {
	...
	var jsonFile map[string]string

	os.MkdirAll(settings.Path, 0755)

	// var translationsFile i18nFile

	//Read the existing json file if it exists.
	if doesFileExist(settings.Path + "/en.json") {
		err = readFileAndParse(settings.Path+"/en.json", &jsonFile)
		if err != nil {
			fmt.Println("Error reading en.json.  " + err.Error())
			return
		}
	} else {
	...
	parseAndWriteFile(settings.Path+"/en.json", jsonFile, 0755)

	// //First add or update the en map.
	// if translationsFile.EN == nil {
	// 	translationsFile.EN = make(map[string]string)
	// }
	// translationsFile.EN[key] = value
	...
	for i := range settings.Languages {
		...
		var jsonTranslationFile map[string]string
		if doesFileExist(settings.Path + "/" + lang + ".json") {
			err = readFileAndParse(settings.Path+"/"+lang+".json", &jsonTranslationFile)
			if err != nil {
				fmt.Println("Error reading en.json.  " + err.Error())
				return
			}
		} else {
			jsonTranslationFile = make(map[string]string)
		}

		jsonTranslationFile[key] = translation.Result()

		parseAndWriteFile(settings.Path+"/"+lang+".json", jsonTranslationFile, 0755)
	}
	// 	switch lang {
	// 	case "az":
	...
	// 		}
	// 		tr	parseAndWriteFile(settings.Path+"/"+page+".json", translationsFile, 0755)anslationsFile.SV[key] = translation.Result()
	// 	case "ja":
